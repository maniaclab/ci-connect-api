...
std::atomic<bool> reaperStop(true);
...
ProcessHandle startProcessAsync(std::string exe, const std::vector<std::string>& args, 
	...
	if(exe.find('/')==std::string::npos){
		...
		while(true){
			...
			if(err==0 && access(posExe.c_str(),R_OK|X_OK)==0){
				exe=posExe;
				break;
			}
			if(next==std::string::npos)
				throw std::runtime_error("Unable to locate "+exe+" in PATH ("+defPath+')');
			idx=next+1;
		}
	}
	else{
		//exe contains a slash, so we assume it a usable path. 
		//Check that the file exists and we can execute it
		struct stat info;
		...
		err=access(exe.c_str(),R_OK|X_OK);
		if(err){
			err=errno;
			throw std::runtime_error("Cannot execute "+exe+": Permission denied");
		}
	}
